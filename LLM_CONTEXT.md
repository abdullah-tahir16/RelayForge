...
Endpoint tests are first-class synthetic delivery traffic, not synchronous URL probes. `POST /api/v1/endpoints/:id/test` creates a reserved `relayforge.endpoint.test` Event plus one targeted Delivery for the selected Endpoint, bypassing subscription routing but reusing the v4 delivery job, signing, retries, attempts, runs, and DLQ behavior. Test Events are identified by internal Event source fields, not client-provided metadata, and dashboard/API responses expose `isTest` plus `testTargetEndpointId`.
